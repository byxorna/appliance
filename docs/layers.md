...
The binding constraint is **meta-seeed-cm4**. This layer provides the `seeed-reterminal` machine — the BSP for the reTerminal's DSI display, touchscreen, buttons, and light sensor. Our build uses `MACHINE=appliance-reterminal`, a derived machine in meta-appliance-os that inherits the Seeed BSP and removes DT overlays incompatible with the pinned 6.1 kernel. meta-seeed-cm4 is maintained by Seeed (the hardware vendor), has no Yocto release branches (only `main`), and declares:
